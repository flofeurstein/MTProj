/**************************************************************************************************
  ...
  Revised:        $Date: 2012-02-26 13:16:49 -0800 (Sun, 26 Feb 2012) $
  Revision:       $Revision: 29524 $
 ...
 * 16-bit MAC timer maximum sleep duration = 0xFFFF/(32.768KHz/8) = 16 seconds
 */
#define MAX_SLEEP_COUNT                  0xFFFF              /* maximum sleep count allowed by H/W */

/* set MSP430 power mode */
#define HAL_SLEEP_TIMER_SLEEP()             st( (macPib.beaconOrder == MAC_BO_NON_BEACON)? \
                                                 __low_power_mode_3() : __low_power_mode_1(); \
                                              )
...
/* convert 320 usec units to msec with round */
#define HAL_SLEEP_320US_TO_MS(u320)         (((((uint32) (u320)) * 8) + 24) / 25)

/* Cannot turn off interrupt for Beacon Enabled MAC */
#define HAL_SLEEP_DISABLE_INTERRUPTS()      st( if (macPib.beaconOrder == MAC_BO_NON_BEACON) HAL_DISABLE_INTERRUPTS(); )

/* Wakeup early so that the radio is powered up before RX is enabled */
#define HAL_BEACON_ENABLE_EARLY_WAKEUP()    ((uint8) ((macPib.beaconOrder == MAC_BO_NON_BEACON) ? 0 : 2 ))
